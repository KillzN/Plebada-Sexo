...
import { PlebadaJadiBot } from './plugins/serbot-jadibot.js'
...
say(`Sigueme en ig como @ra_team.5`, {
  font: 'console',
  align: 'center',
  gradient: ['red', 'magenta']
});
  ...
  if (readRutaJadiBot.length > 0) {
    ...
    for (const gjbts of readRutaJadiBot) {
      ...
      if (readBotPath.includes(creds)) {
        PlebadaJadiBot({ pathPlebadaJadiBot: botPath, m: null, conn, args: '', usedPrefix: '/', command: 'serbot' })
      }
    }
  }
}
...
setInterval(async () => {
  if (!conn?.user || conn?.ws?.readyState !== 1) return // evita ejecutar si el bot está desconectado
